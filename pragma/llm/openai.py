...
class OpenAIProvider:
    ...
    def __init__(
        ...
        if not self.api_key:
            raise LLMError(
                "OPENAI_API_KEY not set. Set OPENAI_API_KEY environment variable or pass api_key."
            )
        self.model = model
    ...
    def complete(self, messages: List[Dict[str, str]], **kwargs: Any) -> str:
        """Synchronous completion with retry (1s/2s/4s backoff)."""
        ...
        raise LLMError(f"OpenAI failed after 3 attempts: {last_error}")
